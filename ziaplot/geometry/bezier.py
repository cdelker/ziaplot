...
from .. import util
from .geometry import PointType, LineType, BezierQuadType, BezierCubicType


def quadratic_xy(b: BezierQuadType, t: float) -> PointType:
    ''' Point on Quadratic Bezier at parameter t '''
    (p1x, p1y), (p2x, p2y), (p3x, p3y) = b
    x = p2x + (1-t)**2 * (p1x - p2x) + t**2 * (p3x - p2x)
    y = p2y + (1-t)**2 * (p1y - p2y) + t**2 * (p3y - p2y)
    return x, y


def quadratic_tangent_slope(b: BezierQuadType, t: float) -> float:
...
def quadtratic_tangent_angle(b: BezierQuadType, t: float) -> float:
...
def cubic_xy(b: BezierCubicType, t: float) -> PointType:
...
def cubic_tangent_slope(b: BezierCubicType, t: float) -> float:
    ''' Slope of tanget at parameter t '''
    (p1x, p1y), (p2x, p2y), (p3x, p3y), (p4x, p4y) = b
...
def cubic_tangent_angle(b: BezierCubicType, t: float) -> float:
    ''' Get angle of tangent at parameter t (radians) '''
    (p1x, p1y), (p2x, p2y), (p3x, p3y) = b
    bprime_x = 3*(1-t)**2 * (p2x-p1x) + 6*(1-t)*t*(p3x-p2x) + 3*t**2*(p4x-p3x)
    bprime_y = 3*(1-t)**2 * (p2y-p1y) + 6*(1-t)*t*(p3y-p2y) + 3*t**2*(p4y-p3y)
...
# Operate on either Cubic or Quadratic:

def xy(b: BezierQuadType | BezierCubicType, t: float) -> PointType:
    ''' Point on a Bezier curve at parameter t '''
    if len(b) == 3:
        return quadratic_xy(b, t)
    return cubic_xy(b, t)


def tangent_slope(b: BezierQuadType | BezierCubicType, t: float) -> float:
    if len(b) == 3:
        return quadratic_tangent_slope(b, t)
    return cubic_tangent_slope(b, t)


def tangent_angle(b: BezierQuadType | BezierCubicType, t: float) -> float:
    if len(b) == 3:
        return quadtratic_tangent_angle(b, t)
    return cubic_tangent_angle(b, t)


def length(b: BezierQuadType | BezierCubicType, n: int = 50) -> float:
    ...
    p = [xy(b, t) for tt in t]
    dists = [util.distance(p[i], p[i+1]) for i in range(n-1)]
    return sum(dists)


def equal_spaced_t(
        b: BezierQuadType | BezierCubicType,
        nsegments: int = 2,
